...
function getSmtpUser() {
  return process.env.SMTP_USER || 'ming.zhao@chivox.com';
}
  ...
  try {
    ...
    if (forwardUrl) {
      ...
      if (!secret) {
        ...
        return {
          success: false,
          error: 'Email service is not configured. Please email ming.zhao@chivox.com directly.',
        };
      }
      ...
      try {
        ...
        return {
          success: false,
          error: 'Couldn’t send right now. Please email ming.zhao@chivox.com directly.',
        };
      } finally {
        clearTimeout(t);
      }
