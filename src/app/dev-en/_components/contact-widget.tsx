 ...
 *
 * Clicking the bubble opens a compact card with the channels a developer
 * actually reaches for: technical support email, sales / enterprise email,
 * and the product docs. Emails get a one-click copy affordance because
 * `mailto:` doesn't always resolve to a configured client.
 *
 * It is anchored above the dev-only language toggle (which lives at
 * `bottom-4 right-4`) so the two never overlap.
 */
...
export function DevEnContactWidget() {
  ...
  // Close on Escape and on click outside the widget.
  useEffect(() => {
    if (!open) return;
    ...
    >
      {open && (
        <div
          ...
          className="w-[min(21rem,calc(100vw-2rem))] origin-bottom-right overflow-hidden rounded-2xl border border-border bg-card text-card-foreground shadow-2xl shadow-black/[0.16] dark:shadow-black/60 ring-1 ring-black/[0.02] animate-in fade-in zoom-in-95 slide-in-from-bottom-2 duration-200"
        >
          {/* Header — emerald wash + status, the Intercom-style "we're here" cue */}
          ...
          <div className="px-2.5 pb-2.5">
            ...
            <EmailRow
              ...
              label={t('Sales & enterprise', '销售与企业合作')}
              email={SALES_EMAIL}
              subject="Chivox MCP inquiry"
              copyLabel={t('Copy email', '复制邮箱')}
              copiedLabel={t('Copied', '已复制')}
            />
      ...
      >
        {open ? (
          ...
          <>
            ...
            {/* Online dot — subtle "we're available" cue on the closed pill */}
